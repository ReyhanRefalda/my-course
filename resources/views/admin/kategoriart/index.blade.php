<x-app-layout>
    ...
    <div class="flex justify-between items-center space-x-4 p-4">
        <div class="w-[300px]">
            <form action="{{ route('admin.kategoriart.index') }}" method="GET" class="m-0 flex items-center">
                <div class="flex items-center space-x-2 bg-white border border-gray-300 rounded-[30px] px-4 py-[2px] shadow-md">
                    <button type="submit" class="text-gray-400">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"></path>
                        </svg>
                    </button>
                    <input type="text" name="search" placeholder="Search Category..." value="{{ request('search') }}" class="block w-full px-4 text-[#898D93] bg-[#fff] border-none focus:ring-0 focus:border-none sm:text-sm">
                </div>
            </form>
        </div>
        
        <button onclick="openModal('create')" class="font-bold py-2 px-6 text-white rounded-full shadow bg-[#3525B3]">Add New</button>
    </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-[30px] p-8">
                <table class="table-auto w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($kategoriarts as $kategoriart)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-4 flex items-center gap-4">
                                    @if($kategoriart->icon)
                                        <img src="{{ asset('storage/' . $kategoriart->icon) }}" alt="{{ $kategoriart->name }}" class="w-12 h-12 rounded-lg object-cover">
                                    @endif
                                    <h3 class="text-gray-900 font-semibold">{{ $kategoriart->name }}</h3>
                                </td>
                                <td class="px-4 py-4 text-gray-700 text-sm">
                                    {{ $kategoriart->created_at->isoFormat('ddd, D/MM/YYYY') }}
                                </td>
                                <td class="p-4 flex justify-end gap-x-2">
                                    <button onclick="openModal('edit', {{ json_encode($kategoriart) }})" class="py-2 px-6 rounded-full bg-[#3525B3] text-white font-semibold">Edit</button>
                                    <form action="{{ route('admin.kategoriart.destroy', $kategoriart->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Yakin ingin menghapus kategori ini?')" class="text-red-500 hover:bg-red-100 p-2 rounded-full">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center py-6">
                                    <img src="{{ asset('assets/images/background/no-data.jpg') }}" alt="No Data" class="mx-auto w-40">
...
function openModal(mode, category = null) {
    if (mode === 'create') {
        ...
        editForm.action = `/admin/kategoriart/${category.id}`;
